...
void HOTEL::checkout() {
    ...
    while (fin.read((char*)this, sizeof(HOTEL))) {
        if (room_no == r) {
            // If room is found, display customer details
            cout << "\n Name: " << name;
            cout << "\n Address: " << address;
            cout << "\n Phone Number: " << phone;
            cout << "\n Room Type: " << rtype;
            ...
            cin >> ch;
            if (ch == 'n') {
                fout.write((char*)this, sizeof(HOTEL)); // Write back if not checking out
            } else {
                cout << "\n Customer Checked Out.";
                found = true; // Mark as found and not write to temp
            }
        } else {
            fout.write((char*)this, sizeof(HOTEL)); // Write other records to temp
        }
    }
    fin.close();
    fout.close();
    ...
    if (!found) {
        ...
        remove("Record.DAT"); // Delete old file
        rename("temp.DAT", "Record.DAT"); // Rename temp to original
    }
    getchar();
...
void HOTEL::main_menu() {
    ...
    while (choice != 7) { // Change condition to 7
        system("clear");
        cout << "\n\t\t\t\t *************";
        cout << "\n\t\t\t\t **THE HOTEL**";
        cout << "\n\t\t\t\t *************";
        ...
        switch (choice) {
            ...
            default:
                ...
                getchar(); // Wait for user input
                getchar(); // This is to consume the newline character after the previous input
        }
    }
}
